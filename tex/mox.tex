...
\[
\mode{once}^{\dagger} = \mode{unique},\quad
\mode{many}^{\dagger} = \mode{aliased},\quad
\mode{non-portable}^{\dagger} = \mode{uncontended},\quad
\mode{portable}^{\dagger} = \mode{contended}.
\]
Joins $\lor$ are computed in the $\leqto$ lattice. Intuitively, a many closure can only reuse captured data as aliased, and a portable closure forces captured bindings to be contended. Locks appear in the $\lambda$-rule above and in constructs such as $\kw{fork}$, which applies $\Lock[(\mode{global}, o, \mode{portable})]{\Gamma}$ to ensure the forked closure is global and portable. Because the lock filters out mode-incompatible bindings and weakens the rest, applying it preserves the semantic interpretation of $\Gamma$; after a portable lock, no binding in the context remains non-portable or uncontended.
