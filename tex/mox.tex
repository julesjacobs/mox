...
\[
\begin{alignedat}{2}
\text{linearity (l):}&\quad \mode{many} \leqto \mode{once} \leqto \mode{never} \\
&\quad \mode{many} \leqin \mode{once} \leqin \mode{never}\\[0.5ex]
\text{portability (p):}&\quad \mode{portable} \leqto \mode{nonportable}\\
&\quad \mode{portable} \leqin \mode{nonportable}\\
\text{areality (a):}&\quad \mode{global} \leqto \mode{borrowed}\\
&\quad \mode{global} \leqin \mode{global},\ \mode{borrowed} \leqin \mode{borrowed}
\end{alignedat}
\]
...
For future modes we additionally track the sentinel $\mode{never}$ on the linearity axis. A $\mode{never}$ function cannot be invoked; operationally it represents a closure whose call capability has been consumed. Aliasing a $\mode{once}$ closure demotes it to $\mode{never}$ rather than rejecting the program outright.
...
\begin{alignat*}{2}
e &\mathrel{::=} {} &\quad& \text{-- expressions}\\
 & \mid x \mid \Let{x}{e_1}{e_2} &\quad& \text{-- variables}\\
 & \mid \UnitTerm \mid \Absurd{e}&\quad& \text{-- unit and empty}\\
 & \mid \App{e_1}{e_2} \mid \Fun{x}{e} &\quad& \text{-- functions}\\
 & \mid \Pair{e_1}{e_2} \mid \LetPair{x_1}{x_2}{e_1}{e_2} &\quad& \text{-- pairs}\\
 & \mid \Inl{e} \mid \Inr{e} &\quad& \text{-- sums}\\
 & \mid \Match{e}{x_1}{e_1}{x_2}{e_2} &\quad&\\
 & \mid \RefCell{e} \mid \Deref{e} \mid \Assign{e_1}{e_2} &\quad& \text{-- references}
\end{alignat*}
...
\begin{alignat*}{2}
\tau &\mathrel{::=} {} &\quad&\\
 & \mid \TUnit \mid \TEmpty &\quad& \text{-- unit and empty}\\
 & \mid \TFun[f]{\tau_1}{\tau_2} &\quad& \text{-- functions}\\
 & \mid \TPair[s]{\tau_1}{\tau_2} &\quad& \text{-- pairs}\\
 & \mid \TSum[s]{\tau_1}{\tau_2} &\quad& \text{-- sums}\\
 & \mid \TRef[r]{\tau} &\quad& \text{-- references}
\end{alignat*}
...
Reference annotations~$r$ decorate mutable references and collect the axes that matter for cells: $r = (\text{uniqueness}, \text{areality}, \text{contention})$ and we write $r_{\text{u}}, r_{\text{a}}, r_{\text{c}}$ for their respective components.
We write $\hat{f}$ for the embedding of $f$ into the full mode lattice (past component~$\bot_{\mathrm{in}}$, future component~$f$), and $\hat{s}$ and $\hat{r}$ for the embeddings of $s$ and $r$ respectively.
Comparisons such as $s_1 \leqto s_2$ or $r_1 \leqin r_2$ are taken componentwise, and joins and meets on $s$ or $r$ use the $\leqin$ order on each axis; we reuse the symbols $\sqcup$ and $\sqcap$ for these operations.
...
Creating a reference defaults to the most precise handle $r_0 = (\mode{unique}, \mode{global}, \mode{uncontended})$, from which weaker capabilities can be derived via subtyping or aliasing.
...
\and
\inferrule
  {\hat{r} \leqin m \\ \tau : m \sqcap \hat{r}}
  {\TRef[r]{\tau} : m}
\end{mathpar}
...
\vspace{0.5em}\textit{References}
\begin{mathpar}
\inferrule
  {\judge{\Gamma}{e}{\tau}}
  {\judge{\Gamma}{\RefCell{e}}{\TRef{\tau}}}
\and
\inferrule
  {\judge{\Gamma}{e}{\TRef{\tau}}}
  {\judge{\Gamma}{\Deref{e}}{\tau}}
\and
\inferrule
  {\judge{\Gamma_1}{e_1}{\TRef{\tau}} \\ \judge{\Gamma_2}{e_2}{\tau}}
  {\judge{\Gamma_1 \uplus \Gamma_2}{\Assign{e_1}{e_2}}{\TUnit}}
\end{mathpar}

We write simply $\RefCell{e}$ when allocating a cell; the resulting handle adopts the most precise capability $r_0 = (\mode{unique}, \mode{global}, \mode{uncontended})$, with weaker modes obtained by subtyping its type.
A dereference yields the stored payload, and assignment consumes the contexts used to establish the reference and the incoming value.
...
\and
\inferrule
  {f = (a, l, p) \and l \in \{\mode{never}, \mode{once}\} \and f' = (a, \mode{never}, p)}
  {\Alias{\TFun[f]{\tau_1}{\tau_2}}{\TFun[f']{\tau_1}{\tau_2}}}
\end{mathpar}
...
\inferrule
  {\Alias{\tau}{\tau'} \\ r = (u, a, c)}
  {\Alias{\TRef[r]{\tau}}{\TRef[(\mode{aliased}, a, c)]{\tau'}}}
\end{mathpar}
...
\begin{align*}
  ...
  \Lock[f]{\TPair[s]{\tau_1}{\tau_2}} &\ =\ \TPair[s']{\Lock[f]{\tau_1}}{\Lock[f]{\tau_2}} \quad\text{where } s' = (s_{\text{u}} \sqcup f_{\text{linearity}}^\dagger,\ s_{\text{a}})\\
  \Lock[f]{\TSum[s]{\tau_1}{\tau_2}} &\ =\ \TSum[s']{\Lock[f]{\tau_1}}{\Lock[f]{\tau_2}} \quad\text{where } s' = (s_{\text{u}} \sqcup f_{\text{linearity}}^\dagger,\ s_{\text{a}})\\
  \Lock[f]{\TRef[r]{\tau}} &\ =\ \TRef[r']{\Lock[f]{\tau}} \quad\text{where } r' = (r_{\text{u}} \sqcup f_{\text{linearity}}^\dagger,\ r_{\text{a}},\ r_{\text{c}} \sqcup f_{\text{portability}}^\dagger)\\
...
\mode{nonportable}^{\dagger} &= \mode{uncontended} &
...
If we lock a type with respect to $\mode{many}$ then we change all $\mode{unique}$ to $\mode{aliased}$.
If we lock a type with respect to $\mode{portable}$ then we change all $\mode{uncontended}$ and $\mode{shared}$ to $\mode{contended}$.

\section{Type Inference}

We must be able to maintain these constraints on \textbf{type variables}:
\begin{itemize}
  \item Subtyping constraints $\Sub{\tau_1}{\tau_2}$ between two type variables.
  \item Alias constraints between two type variables ("type variable $\tau_1$ is the aliased version of type variable $\tau_2$").
  \item Lock constraints between two type variables ("type variable $\tau_1$ is type variable $\tau_2$ locked with modes $f$").
  \item In-placement constraints on a type variable (given by a set of modes $m$).
\end{itemize}

\noindent
We must able able to maintain these constraints on \textbf{mode variables}:
\begin{itemize}
  \item $\leqin$ constraints between two mode variables.
  \item $\leqto$ constraints between two mode variables.
  \item Domain constraints on mode variables.
  \item Dagger-constraints between mode variables.
\end{itemize}
